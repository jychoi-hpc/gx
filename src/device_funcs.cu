...
__device__ cufftCallbackStoreC   i_kzLinkedNTFT_callbackPtr = i_kzLinkedNTFT;  // JMH
			       ...
			       cuComplex* GRhs)  // added nonTwist portion // JMH
{
  ...
  if (ikx[0] < 0) { // because we set this negative for NTFT, this is essentially if (nonTwist)
    ...
    if (idp < nLinks && idn < nChains && idlm < nMoms) {
      ...
      // note: idzl(conventional) = idp(NTFT) = grid point number in chain and idk is similar to idpn but idpn has a value for every z
			      ...
			      const float gradpar, const specie sp, cuComplex* __restrict__ rhs_par)
{
  ...
  if ((idy < nyc) && (idx < nx) && unmasked(idx, idy) && (idzl < nz*nl)) {
	...
	rhs_par[globalIdx] = rhs_par[globalIdx] - Jflr(l, b_s) * phi_ * zt_ * vt_ * gradpar - JflrB(l, b_s) * bpar_ * vt_ * gradpar;
      }
