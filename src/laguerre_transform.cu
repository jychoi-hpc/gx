...
void LaguerreTransform::transformToGrid(float* G_in, float* g_res)
{
  ...
  if( status != CUBLAS_STATUS_SUCCESS )
    fprintf( stderr, "ERROR in transformToGrid in cublasSgemmStridedBatched (%d:)", status );
}
...
void LaguerreTransform::transformToSpectral(float* g_in, float* G_res)
{
  ...
  cublasStatus_t status = cublasSgemmStridedBatched(handle, CUBLAS_OP_N, CUBLAS_OP_N,
     ...
     batch_size_);
  if( status != CUBLAS_STATUS_SUCCESS )
    fprintf( stderr, "ERROR in transformToSpectral in cublasSgemmStridedBatched (%d:)", status);
}
