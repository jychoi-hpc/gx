 ...
 : pars_(pars), grids_(grids)
{
  ...
  dG_inv       = nullptr;  dB_inv       = nullptr;
  Gr_linked    = nullptr;  phi_linked   = nullptr;
  apar_linked  = nullptr;  bpar_linked  = nullptr;
  qneutDenom_linked = nullptr;
  G2_linked = nullptr;     Gr2_linked   = nullptr;
  Gc_linked = nullptr;     Gc2_linked   = nullptr;
  phi2_linked = nullptr;   apar2_linked = nullptr;
  ikxLinked_lw = nullptr;
  dG_back      = nullptr;  dB_back      = nullptr;
  p_map = nullptr; n_map = nullptr; c_map = nullptr; nLinks_map = nullptr; nChains_map = nullptr;
 
  zft_plan_forward           = nullptr;
  zft_plan_inverse           = nullptr;

  zft_plan_forward_singlemom = nullptr;
  zft_plan_forward_laguerre = nullptr;
  ...
  int *nLinks_map_h = (int*) malloc(sizeof(int)*naky*nakx);
  int *nChains_map_h = (int*) malloc(sizeof(int)*naky*nakx);
  int *n_map_h = (int*) malloc(sizeof(int)*naky*nakx);
  int *c_map_h = (int*) malloc(sizeof(int)*naky*nakx);
  ...
  ikxLinked_lw_h = (int**) malloc(sizeof(int*)*nClasses);
  ikxLinked_lw = (int**) malloc(sizeof(int*)*nClasses);

  for(int c=0; c<nClasses; c++) {
    ikxLinked_h[c] = (int*) malloc(sizeof(int)*nLinks[c]*nChains[c]);
    ikyLinked_h[c] = (int*) malloc(sizeof(int)*nLinks[c]*nChains[c]);
    ikxLinked_lw_h[c] = (int*) malloc(sizeof(int)*nLinks[c]);
    checkCuda(cudaMalloc((void**) &ikxLinked_lw[c], sizeof(int)*nLinks[c]));

  }

  kFill(nClasses, nChains, nLinks, ikyLinked_h, ikxLinked_h, linksL, linksR, idxRight, nLinks_map_h, nChains_map_h, n_map_h, c_map_h, naky, nakx);


  dG = (dim3*) malloc(sizeof(dim3)*nClasses);
  dB = (dim3*) malloc(sizeof(dim3)*nClasses);
  dG_inv = (dim3*) malloc(sizeof(dim3)*nClasses);
  dB_inv = (dim3*) malloc(sizeof(dim3)*nClasses);
  dG_inv_l = (dim3*) malloc(sizeof(dim3)*nClasses);
  dB_inv_l = (dim3*) malloc(sizeof(dim3)*nClasses);
  dG_inv_lw = (dim3*) malloc(sizeof(dim3)*nClasses);
  dB_inv_lw = (dim3*) malloc(sizeof(dim3)*nClasses);
  dG_cp_lw = (dim3*) malloc(sizeof(dim3)*nClasses);
  dB_cp_lw = (dim3*) malloc(sizeof(dim3)*nClasses);
  ...
  zft_plan_forward_laguerre = (cufftHandle*) malloc(sizeof(cufftHandle*)*nClasses);
  ...
  Gr_linked = (cuComplex**) malloc(sizeof(cuComplex*)*nClasses);
  G2_linked = (cuComplex**) malloc(sizeof(cuComplex*)*nClasses);
  Gr2_linked = (cuComplex**) malloc(sizeof(cuComplex*)*nClasses);
  Gc_linked = (cuComplex**) malloc(sizeof(cuComplex*)*nClasses);
  Gc2_linked = (cuComplex**) malloc(sizeof(cuComplex*)*nClasses);
  phi_linked = (cuComplex**) malloc(sizeof(cuComplex*)*nClasses);
  phi2_linked = (cuComplex**) malloc(sizeof(cuComplex*)*nClasses);

  apar_linked = (cuComplex**) malloc(sizeof(cuComplex*)*nClasses);
  apar2_linked = (cuComplex**) malloc(sizeof(cuComplex*)*nClasses);
  bpar_linked = (cuComplex**) malloc(sizeof(cuComplex*)*nClasses);
  ...
  max_Jflr_linked = (float**) malloc(sizeof(float*)*nClasses);
  max_Jflr2_linked = (float**) malloc(sizeof(float*)*nClasses);

  if(pars_->fapar > 0.){
    max_ampereParFac_inv_linked = (float**) malloc(sizeof(float*)*nClasses);
  }
  if(pars_->fbpar > 0.){
    max_qneutFacBpar_inv_linked = (float**) malloc(sizeof(float*)*nClasses);
    max_amperePerpFacPhi_inv_linked = (float**) malloc(sizeof(float*)*nClasses);
    max_amperePerpFacBpar_inv_linked = (float**) malloc(sizeof(float*)*nClasses);
  }
  ...
  stream = (cudaStream_t*) malloc(sizeof(cudaStream_t)*nClasses);
  ...
  for(int c=0; c<nClasses; c++) {
    ...
    for(int ik = 0; ik < nLinks[c]; ik++){
      if(c == 0){
        ikxLinked_lw_h[c][ik] = ik;
      }
      else{
	if(ik == 0){
	  ikxLinked_lw_h[c][ik] = ikxLinked_lw_h[c-1][nLinks[c-1]-1] + 1;
	}
	else{
          ikxLinked_lw_h[c][ik] = ikxLinked_lw_h[c][ik-1] + 1;
	}
      }
    }
    checkCuda(cudaMemcpy(ikxLinked_lw[c], ikxLinked_lw_h[c], sizeof(int)*nLinks[c], cudaMemcpyHostToDevice));
    ...
    cudaMemset(phi2_linked[c], 0., sLClmz);
    checkCuda(cudaMalloc((void**) &apar2_linked[c], sLClmz));
    cudaMemset(apar2_linked[c], 0., sLClmz);

    checkCuda(cudaMalloc((void**) &apar_linked[c], sLClmz));
    cudaMemset(apar_linked[c], 0., sLClmz);
    checkCuda(cudaMalloc((void**) &bpar_linked[c], sLClmz));
    cudaMemset(bpar_linked[c], 0., sLClmz);

    checkCuda(cudaMalloc((void**) &G2_linked[c], sLClmz));
    cudaMemset(G2_linked[c], 0., sLClmz);
    checkCuda(cudaMalloc((void**) &qneutDenom_linked[c], sLClmz_f));
    cudaMemset(qneutDenom_linked[c], 0., sLClmz_f);

    if(pars_->implicit_preconditioner=="long_wavelength"){
      checkCuda(cudaMalloc((void**) &Gc_linked[c], sLClmz_lw));
      cudaMemset(Gc_linked[c], 0., sLClmz_lw);
      checkCuda(cudaMalloc((void**) &Gc2_linked[c], sLClmz_lw));
      cudaMemset(Gc2_linked[c], 0., sLClmz_lw);
    }
    else{
      checkCuda(cudaMalloc((void**) &Gc_linked[c], sLClmz));
      cudaMemset(Gc_linked[c], 0., sLClmz);
      checkCuda(cudaMalloc((void**) &Gc2_linked[c], sLClmz));
      cudaMemset(Gc2_linked[c], 0., sLClmz);
   
    }

    checkCuda(cudaMalloc((void**) &Gr_linked[c], sLClmz));
    cudaMemset(Gr_linked[c], 0., sLClmz);
    checkCuda(cudaMalloc((void**) &Gr2_linked[c], sLClmz));
    cudaMemset(Gr2_linked[c], 0., sLClmz);
    ...
    checkCuda(cudaMalloc((void**) &max_Jflr_linked[c], sLClmz_f));
    cudaMemset(max_Jflr_linked[c], 0., sLClmz_f);
    checkCuda(cudaMalloc((void**) &max_Jflr2_linked[c], sLClmz_f));
    cudaMemset(max_Jflr2_linked[c], 0., sLClmz_f);



    if(pars_->fapar > 0.){
      checkCuda(cudaMalloc((void**) &max_ampereParFac_inv_linked[c], sLClmz_f));
      cudaMemset(max_ampereParFac_inv_linked[c], 0., sLClmz_f);
    }
    
    if(pars_->fbpar > 0.){
      checkCuda(cudaMalloc((void**) &max_qneutFacBpar_inv_linked[c], sLClmz_f));
      cudaMemset(max_qneutFacBpar_inv_linked[c], 0., sLClmz_f);
      checkCuda(cudaMalloc((void**) &max_amperePerpFacPhi_inv_linked[c], sLClmz_f));
      cudaMemset(max_amperePerpFacPhi_inv_linked[c], 0., sLClmz_f);
      checkCuda(cudaMalloc((void**) &max_amperePerpFacBpar_inv_linked[c], sLClmz_f));
      cudaMemset(max_amperePerpFacBpar_inv_linked[c], 0., sLClmz_f);  
    }
    ...
    int nn1, nn2, nn3, nt1, nt2, nt3, nb1, nb2, nb3, nn4, nn5, nt4, nt5, nb4, nb5;
    ...
    nn4 = grids_->Nl;                   nt4 = min( nn4,  1 );    nb4 = 1 + (nn4-1)/nt4;
    nn5 = nLinks[c];                    nt5 = min( nn5,  512/WARPSIZE );    nb5 = 1 + (nn5-1)/nt5;
    ...
    dB_inv_l[c] = dim3(nt1, nt2, 1);
    dG_inv_l[c] = dim3(nb1, nb2, 1);
    dB_inv_lw[c] = dim3(nt1, nt5, nt4);
    dG_inv_lw[c] = dim3(nb1, nb5, nb4);
    dB_cp_lw[c] = dim3(nt1, nt5, nt3);
    dG_cp_lw[c] = dim3(nb1, nb5, nb3);
  ...
  for(int c=0; c<nClasses; c++) {
    ...
    cufftCreate(    &zft_plan_forward_laguerre[c]);
    ...
    checkCuda(cufftMakePlanMany(zft_plan_forward_laguerre[c], 1, &size, NULL, 1, 0, NULL, 1, 0, CUFFT_C2C, nChains[c]*grids_->Nl, &workSize));
...
/*    init_kzLinked <<<1,1>>> (kzLinked[c], nLinks[c], false);
    ...
    int nn1, nn2, nn3, nn4, nt1, nt2, nt3, nt4, nb1, nb2, nb3, nb4, nn5, nt5, nb5;
    ...
    nn4 = grids_->Nl;                   nt4 = min( nn4,  4 );    nb4 = 1 + (nn4-1)/nt4;
    nn5 = nLinks[c];                    nt5 = min( nn5,  4 );    nb5 = 1 + (nn5-1)/nt5;
    ...
    cufftSetStream(zft_plan_forward[c], stream[c]);
    cufftSetStream(zft_plan_inverse[c], stream[c]);
    cufftSetStream(zft_plan_forward_laguerre[c], stream[c]);
...
GradParallelLinked::~GradParallelLinked()
{
  ...
  if (dB)      free(dB);
  if (dG)      free(dG);
  if (dB_inv)      free(dB_inv);
  if (dG_inv)      free(dG_inv);
  if (dB_back)      free(dB_back);
  if (dG_back)      free(dG_back);
  ...
  for(int c=0; c<nClasses; c++) {
    ...
    cufftDestroy(    zft_plan_forward_laguerre[c]);
    ...
    if (Gr_linked[c])          cudaFree(Gr_linked[c]);
    if (G2_linked[c])          cudaFree(G_linked[c]);
    if (Gr2_linked[c])          cudaFree(Gr_linked[c]);
    if (Gc_linked[c])          cudaFree(Gc_linked[c]);
    if (Gc2_linked[c])          cudaFree(Gc2_linked[c]);

    if (phi_linked[c])          cudaFree(phi_linked[c]);
    if (phi2_linked[c])          cudaFree(phi2_linked[c]);
    if (apar2_linked[c])          cudaFree(apar2_linked[c]);
    if (apar_linked[c])          cudaFree(apar_linked[c]);
    if (bpar_linked[c])          cudaFree(bpar_linked[c]);
    ...
    if (max_Jflr_linked[c])			 cudaFree(max_Jflr_linked[c]);
    if (max_Jflr2_linked[c])			 cudaFree(max_Jflr2_linked[c]);
    ...
    if (max_amperePerpFacBpar_inv_linked[c])          cudaFree(max_amperePerpFacBpar_inv_linked[c]);
  ...
  if (zft_plan_forward_laguerre)    free(    zft_plan_forward_laguerre);
  ...
  if (Gr_linked)            free(Gr_linked);
  if (Gc_linked)            free(Gc_linked);
  if (Gc2_linked)            free(Gc2_linked);
  if (G2_linked)            free(G2_linked);
  if (Gr2_linked)            free(Gr2_linked);
  if (phi_linked)            free(phi_linked);
  if (phi2_linked)            free(phi2_linked);
  if (apar2_linked)            free(apar2_linked);
  if (apar_linked)            free(apar_linked);
  if (bpar_linked)            free(bpar_linked);
  ...
  if(max_Jflr_linked)			      free(max_Jflr_linked);
  if(max_Jflr2_linked)   		      free(max_Jflr2_linked);
  ...
  if (max_amperePerpFacBpar_inv_linked)            free(max_amperePerpFacBpar_inv_linked);

  if (kzLinked)            free(kzLinked);
  if (p_map)  cudaFree(p_map);
  if (n_map)  cudaFree(n_map);
  if (c_map)  cudaFree(c_map);
  if (nLinks_map)  cudaFree(nLinks_map);
  if (nChains_map)  cudaFree(nChains_map);
}
...
void GradParallelLinked::zft_streaming_invert_full_em(MomentsG** G, cuComplex** Gc, cuComplex** Gr, MomentsG** G2, cuComplex** phi, cuComplex** apar, const float* max_qneutFacPhi_inv, const float* max_ampereParFac_inv, const specie spi, const specie spe, const double sdt, const float beta, const float gradpar, bool full_phi) 
{
  for(int c=0; c<nClasses; c++) {

    linkedCopy GCHAINS (G[0]->G(), G_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nmoms, 1./(grids_->Nz*nLinks[c]));
    checkCuda(cufftExecC2C (zft_plan_forward[c], G_linked[c], G_linked[c], CUFFT_FORWARD));
    linkedCopy GCHAINS (G[1]->G(), G2_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nmoms, 1./(grids_->Nz*nLinks[c]));
    checkCuda(cufftExecC2C (zft_plan_forward[c], G2_linked[c], G2_linked[c], CUFFT_FORWARD));

    linkedCopy GCHAINS (G2[0]->G(), Gr_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nmoms, 1./(grids_->Nz*nLinks[c]));
    checkCuda(cufftExecC2C (zft_plan_forward[c], Gr_linked[c], Gr_linked[c], CUFFT_FORWARD));
    linkedCopy GCHAINS (G2[1]->G(), Gr2_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nmoms, 1./(grids_->Nz*nLinks[c]));
    checkCuda(cufftExecC2C (zft_plan_forward[c], Gr2_linked[c], Gr2_linked[c], CUFFT_FORWARD));


    linkedCopy GCHAINS (phi[0], phi_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nl, 1./(grids_->Nz*nLinks[c]));
    cufftExecC2C(zft_plan_forward_laguerre[c], phi_linked[c], phi_linked[c], CUFFT_FORWARD);
    linkedCopy GCHAINS (phi[1], phi2_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nl, 1./(grids_->Nz*nLinks[c]));
    cufftExecC2C(zft_plan_forward_laguerre[c], phi2_linked[c], phi2_linked[c], CUFFT_FORWARD);

    linkedCopy GCHAINS (apar[0], apar_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nl, 1./(grids_->Nz*nLinks[c]));
    cufftExecC2C(zft_plan_forward_laguerre[c], apar_linked[c], apar_linked[c], CUFFT_FORWARD);
    linkedCopy GCHAINS (apar[1], apar2_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nl, 1./(grids_->Nz*nLinks[c]));
    cufftExecC2C(zft_plan_forward_laguerre[c], apar2_linked[c], apar2_linked[c], CUFFT_FORWARD);
    ...
    tridiag_streaming_linked_full_em<<<dG_inv[c], dB_inv[c], 0, stream[c]>>>(G_linked[c], G2_linked[c], Gr_linked[c], Gr2_linked[c], phi_linked[c], phi2_linked[c], apar_linked[c], apar2_linked[c], kzLinked[c], max_qneutFacPhi_inv_linked[c], max_ampereParFac_inv_linked[c], spi, spe, sdt, beta, gradpar, 0, full_phi, nLinks[c], nChains[c]);  
    
    linkedCopy_lw <<<dG_cp_lw[c], dB_cp_lw[c], 0, stream[c]>>> (Gc[0], Gc_linked[c], nLinks[c], ikxLinked_lw[c], grids_->Nmoms);
    linkedCopy_lw <<<dG_cp_lw[c], dB_cp_lw[c], 0, stream[c]>>> (Gc[1], Gc2_linked[c], nLinks[c], ikxLinked_lw[c], grids_->Nmoms);
    linkedCopy_lw <<<dG_cp_lw[c], dB_cp_lw[c], 0, stream[c]>>> (Gr[0], Gr_linked[c], nLinks[c], ikxLinked_lw[c], grids_->Nmoms);
    linkedCopy_lw <<<dG_cp_lw[c], dB_cp_lw[c], 0, stream[c]>>> (Gr[1], Gr2_linked[c], nLinks[c], ikxLinked_lw[c], grids_->Nmoms);


    sherman_morrison_linked_full_em<<<dG_inv[c], dB_inv[c], 0, stream[c]>>>(G_linked[c], G2_linked[c], Gc_linked[c], Gc2_linked[c], Gr_linked[c], Gr2_linked[c], kzLinked[c], max_qneutFacPhi_inv_linked[c], max_ampereParFac_inv_linked[c], spi, spe, sdt, beta, gradpar, nLinks[c], nChains[c]);  

    linkedCopyBack GCHAINS (G_linked[c], G[0]->G(), nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nmoms);
    linkedCopyBack GCHAINS (G2_linked[c], G[1]->G(), nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nmoms);

  }
}

void GradParallelLinked::zft_streaming_invert_full_laguerre(MomentsG** G, cuComplex** Gc, cuComplex** Gr, MomentsG** G2, cuComplex** phi,float** max_qneutFacPhi_inv_l, float** max_Jflr, const specie spi, const specie spe, const double sdt, const float gradpar, bool full_phi) 
{
  for(int c=0; c<nClasses; c++) {

    linkedCopy GCHAINS (G[0]->G(), G_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nmoms);
    checkCuda(cufftExecC2C (zft_plan_forward[c], G_linked[c], G_linked[c], CUFFT_FORWARD));
    linkedCopy GCHAINS (G[1]->G(), G2_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nmoms);
    checkCuda(cufftExecC2C (zft_plan_forward[c], G2_linked[c], G2_linked[c], CUFFT_FORWARD));
    ...
    checkCuda(cufftExecC2C (zft_plan_forward[c], Gr_linked[c], Gr_linked[c], CUFFT_FORWARD));
    linkedCopy GCHAINS (G2[1]->G(), Gr2_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nmoms);
    checkCuda(cufftExecC2C (zft_plan_forward[c], Gr2_linked[c], Gr2_linked[c], CUFFT_FORWARD));
  

    linkedCopy GCHAINS (phi[0], phi_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nl);
    cufftExecC2C(zft_plan_forward_laguerre[c], phi_linked[c], phi_linked[c], CUFFT_FORWARD);
    linkedCopy GCHAINS (phi[1], phi2_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nl);
    cufftExecC2C(zft_plan_forward_laguerre[c], phi2_linked[c], phi2_linked[c], CUFFT_FORWARD);
    ...
    linkedCopy_f GCHAINS (max_Jflr[0], max_Jflr_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nl, 1);
    linkedCopy_f GCHAINS (max_Jflr[1], max_Jflr2_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nl, 1);


    tridiag_streaming_linked_full_laguerre<<<dG_inv_l[c], dB_inv_l[c]>>>(G_linked[c], G2_linked[c], Gr_linked[c], Gr2_linked[c], phi_linked[c], phi2_linked[c], kzLinked[c], max_qneutFacPhi_inv_l_linked[c], max_qneutFacPhi_inv_l2_linked[c], max_Jflr_linked[c], max_Jflr2_linked[c], spi, spe, sdt, gradpar, 0, full_phi, nLinks[c], nChains[c], grids_->Nl, grids_->Nm);  

    sherman_morrison_linked_full_laguerre<<<dG_inv_l[c], dB_inv_l[c]>>>(G_linked[c], G2_linked[c], Gc_linked[c], Gc2_linked[c], Gr_linked[c], Gr2_linked[c], kzLinked[c], max_qneutFacPhi_inv_l_linked[c], max_qneutFacPhi_inv_l2_linked[c], spi, spe, sdt, gradpar, nLinks[c], nChains[c]);  
...
{
  for(int c=0; c<nClasses; c++) {
    sherman_morrison_subsolve_linked_lw<<<dG_inv_lw[c], dB_inv_lw[c], 0, stream[c]>>>(Gc_linked[c], Gc2_linked[c], kzLinked[c], spi, spe, sdt, gradpar, stage, nLinks[c], nChains[c], hyperc, hyper_kz, nu_hyper_l, nu_hyper_m, nu_hyper_lm, p_hyper_l, p_hyper_m, p_hyper_lm, vt_max, dt);

    linkedCopyBack_lw <<<dG_cp_lw[c], dB_cp_lw[c], 0, stream[c]>>> (Gc_linked[c], Gc[0], nLinks[c], ikxLinked_lw[c], grids_->Nmoms);
    linkedCopyBack_lw <<<dG_cp_lw[c], dB_cp_lw[c], 0, stream[c]>>> (Gc2_linked[c], Gc[1], nLinks[c], ikxLinked_lw[c], grids_->Nmoms);

  }
}


void GradParallelLinked::zft_sherman_morrison_subsolve(cuComplex** Gc, float** max_Jflr, const specie spi, const specie spe, const double sdt, const float gradpar) 
{
  for(int c=0; c<nClasses; c++) {
    linkedCopy_f GCHAINS (max_Jflr[0], max_Jflr_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nl, 1);
    linkedCopy_f GCHAINS (max_Jflr[1], max_Jflr2_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nl, 1);

    sherman_morrison_subsolve_laguerre<<<dG_inv_l[c], dB_inv_l[c]>>>(Gc_linked[c], Gc2_linked[c], kzLinked[c], max_Jflr_linked[c], max_Jflr2_linked[c], spi, spe, sdt, gradpar, 0, nLinks[c], nChains[c]);

    linkedCopyBack GCHAINS (Gc_linked[c], Gc[0], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nmoms);
    linkedCopyBack GCHAINS (Gc2_linked[c], Gc[1], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nmoms);
...
/*    linkedCopy_lw <<<dG_cp_lw[c], dB_cp_lw[c]>>> (Gc[0], Gc_linked[c], nLinks[c], ikxLinked_lw[c], grids_->Nmoms);
    linkedCopy_lw <<<dG_cp_lw[c], dB_cp_lw[c]>>> (Gc[1], Gc2_linked[c], nLinks[c], ikxLinked_lw[c], grids_->Nmoms);*/
    linkedCopy_lw <<<dG_cp_lw[c], dB_cp_lw[c], 0, stream[c]>>> (Gc[0], Gc_linked[c], nLinks[c], ikxLinked_lw[c], grids_->Nmoms);
    linkedCopy_lw <<<dG_cp_lw[c], dB_cp_lw[c], 0, stream[c]>>> (Gc[1], Gc2_linked[c], nLinks[c], ikxLinked_lw[c], grids_->Nmoms);
    ...
    tridiag_streaming_linked_full<<<dG_inv[c], dB_inv[c], 0, stream[c]>>>(G_linked[c], G2_linked[c], Gr_linked[c], Gr2_linked[c], phi_linked[c], phi2_linked[c], kzLinked[c], max_qneutFacPhi_inv_linked[c], spi, spe, sdt, gradpar, 0, full_phi, nLinks[c], nChains[c], hyperc, hyper_kz, nu_hyper_l, nu_hyper_m, nu_hyper_lm, p_hyper_l, p_hyper_m, p_hyper_lm, vt_max, dt);  

    sherman_morrison_linked_full<<<dG_inv[c], dB_inv[c], 0, stream[c]>>>(G_linked[c], G2_linked[c], Gc_linked[c], Gc2_linked[c], kzLinked[c], max_qneutFacPhi_inv_linked[c], spi, spe, sdt, gradpar, nLinks[c], nChains[c]); 
...
//  linkedCopyBackAll <<< dG_all, dB_all>>> (G_linked_d, G[0]->G(), p_map, n_map, c_map, nLinks_map, nChains_map, grids_->Nmoms);

}


void GradParallelLinked::zft_streaming_invert(MomentsG* G, MomentsG* Gr, cuComplex* phi, const float* qneutDenom, const float* max_qneutFacPhi_inv, const double sdtvt, const float gradpar, bool full_phi) 
{
  for(int c=0; c<nClasses; c++) {
    ...
    tridiag_streaming_linked<<<dG_inv[c], dB_inv[c]>>>(G_linked[c], Gr_linked[c],phi_linked[c], kzLinked[c], max_qneutFacPhi_inv_linked[c], *(G->species), sdtvt, gradpar, full_phi, nLinks[c], nChains[c]);  
...
void GradParallelLinked::zft_streaming_invert_apar(MomentsG* G, MomentsG* Gr, cuComplex* phi, cuComplex* apar, const float* max_qneutFacPhi_inv, const float* max_ampereParFac_inv, const double sdtvt, const float gradpar, bool full_phi, bool hypercollisions_kz, int p_hyper_m, float nu_hyper_m) 
{
  ...
  for(int c=0; c<nClasses; c++) {
    ...
    tridiag_streaming_linked_apar<<<dG_inv[c], dB_inv[c]>>>(G_linked[c], Gr_linked[c],phi_linked[c], apar_linked[c], kzLinked[c], max_qneutFacPhi_inv_linked[c], max_ampereParFac_inv_linked[c], pars_->beta,*(G->species), sdtvt, gradpar, full_phi, nLinks[c], nChains[c], hypercollisions_kz, p_hyper_m, nu_hyper_m);
...
{
  for(int c=0; c<nClasses; c++) {
    linkedCopy GCHAINS (G->G(), G_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nmoms, 1./(grids_->Nz*nLinks[c]));

    checkCuda(cufftExecC2C (zft_plan_forward[c], G_linked[c], G_linked[c], CUFFT_FORWARD)); // normalization included in linkedCopy
  }
  linkedCopyBackAll <<< dG_all, dB_all >>> (G_linked_d, G->G(), p_map, n_map, c_map, nLinks_map, nChains_map, grids_->Nmoms);
}
...
void GradParallelLinked::zft_inverse(MomentsG* G) 
{
  for(int c=0; c<nClasses; c++) {
    ...
    checkCuda(cufftExecC2C (zft_plan_inverse[c], G_linked[c], G_linked[c], CUFFT_INVERSE));
  }
  linkedCopyBackAll <<< dG_all, dB_all >>> (G_linked_d, G->G(), p_map, n_map, c_map, nLinks_map, nChains_map, grids_->Nmoms);
}
...
void GradParallelLinked::zft(cuComplex* m, cuComplex* res)
{
  ...
  for(int c=0; c<nClasses; c++) {  // these only use the G(0,0) part of G_linked
    linkedCopy GCHAINS (m, G_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], nMoms, 1./(grids_->Nz*nLinks[c]));

    checkCuda(cufftExecC2C(zft_plan_forward_singlemom[c], G_linked[c], G_linked[c], CUFFT_FORWARD)); // normalization included in linkedCopy
  }
  linkedCopyBackAll <<< dG_all, dB_all >>> (G_linked_d, res, p_map, n_map, c_map, nLinks_map, nChains_map, grids_->Nmoms);
}

void GradParallelLinked::zft_nmoms(cuComplex* m, cuComplex* res, int nMoms)
{
  for(int c=0; c<nClasses; c++) {  // these only use the G(0,0) part of G_linked
    linkedCopy GCHAINS (m, G_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], nMoms);

    cufftExecC2C(zft_plan_forward_laguerre[c], G_linked[c], G_linked[c], CUFFT_FORWARD);
...
// for a single moment m 
void GradParallelLinked::dz(cuComplex* m, cuComplex* res, bool accumulate)
{
  int nMoms=1;
...
void GradParallelLinked::abs_dz(MomentsG* G, MomentsG* res, bool accumulate) 
{
  for(int c=0; c<nClasses; c++) {
    // each "class" has a different number of links in the chains, and a different number of chains.
    linkedCopy GCHAINS (G->G(), G_linked[c], nLinks[c], nChains[c], ikxLinked[c], ikyLinked[c], grids_->Nmoms);
...
// for a single moment m 
void GradParallelLinked::dz2(cuComplex* m, cuComplex* res)
{
  int nMoms=1;
...
// for a single moment m
void GradParallelLinked::abs_dz(cuComplex* m, cuComplex* res, bool accumulate)
{
  int nMoms=1;
...
void GradParallelLinked::set_callbacks()
{
  ...
  checkCuda(cudaMemcpyFromSymbol(&mkz2_Linked_callbackPtr_h, 
                     ...
                     sizeof(mkz2_Linked_callbackPtr_h)));
  ...
  for(int c=0; c<nClasses; c++) {
    ...
    checkCuda(cufftXtSetCallback(    hyperz_plan_forward[c],
		       (void**)   &hyperkzLinked_callbackPtr_h, CUFFT_CB_ST_COMPLEX, (void**)&hyperdata_d));
    ...
    checkCuda(cufftXtSetCallback(    dz2_plan_forward[c],
		       (void**)   &mkz2_Linked_callbackPtr_h, CUFFT_CB_ST_COMPLEX, (void**)&kzLinked[c]));
