...
void IMEX_3stage_Full::invert_implicit_terms(MomentsG** G1, MomentsG** Gc, MomentsG** Gr, Fields *f, double sdt,const float gradpar_, const float* bmagInv_, int ielectron)
{
  
/*  tridiag_streaming_periodic_full<<<dG, dB>>>(G1[0]->G(), f->phi, grids_->kz, solver_->get_max_qneutFacPhi_inv(), *(G1[0]->species), sdt, gradpar_, 0);
  tridiag_streaming_periodic_full<<<dG, dB>>>(G1[ielectron]->G(), f->phi, grids_->kz, solver_->get_max_qneutFacPhi_inv(), *(G1[ielectron]->species), sdt, gradpar_, 0);

  tridiag_streaming_periodic_full<<<dG, dB>>>(Gc[0]->G(), f->phi, grids_->kz, solver_->get_max_qneutFacPhi_inv(), *(Gc[0]->species), sdt, gradpar_, 1);
  tridiag_streaming_periodic_full<<<dG, dB>>>(Gc[ielectron]->G(), f->phi, grids_->kz, solver_->get_max_qneutFacPhi_inv(), *(Gc[ielectron]->species), sdt, gradpar_, 1);

  tridiag_streaming_periodic_full<<<dG, dB>>>(Gr[0]->G(), f->phi, grids_->kz, solver_->get_max_qneutFacPhi_inv(), *(Gr[0]->species), sdt, gradpar_, 2);
  tridiag_streaming_periodic_full<<<dG, dB>>>(Gr[ielectron]->G(), f->phi, grids_->kz, solver_->get_max_qneutFacPhi_inv(), *(Gr[ielectron]->species), sdt, gradpar_, 2);*/
...
//    grad_par->zft(Gr[is]);
  ...
  tridiag_streaming_periodic_full<<<dG, dB>>>(G1[0]->G(), G1[ielectron]->G(), f->phi, grids_->kz, solver_->get_max_qneutFacPhi_inv(), *(G1[0]->species), *(G1[ielectron]->species), sdt, gradpar_, 0);
  tridiag_streaming_periodic_full<<<dG, dB>>>(Gc[0]->G(), Gc[ielectron]->G(), f->phi, grids_->kz, solver_->get_max_qneutFacPhi_inv(), *(Gc[0]->species), *(Gc[ielectron]->species), sdt, gradpar_, 1);
  tridiag_streaming_periodic_full<<<dG, dB>>>(Gr[0]->G(), Gr[ielectron]->G(), f->phi, grids_->kz, solver_->get_max_qneutFacPhi_inv(), *(Gr[0]->species), *(Gr[ielectron]->species), sdt, gradpar_, 2);


  sherman_morrison_full<<<dG, dB>>>(G1[0]->G(), G1[ielectron]->G(), Gc[0]->G(), Gc[ielectron]->G(), Gr[0]->G(), Gr[ielectron]->G(), solver_->get_max_qneutFacPhi_inv(), *(G1[0]->species), *(G1[ielectron]->species), gradpar_);

  for(int is = 0; is < grids_->Nspecies; is++){
    grad_par->zft_inverse(G1[is]);
//    grad_par->zft_inverse(Gc[is]);
//    grad_par->zft_inverse(Gr[is]);
  }
...
//    invert_implicit_terms(G1[ielectron], f, p_*dt_,gradpar_);
    invert_implicit_terms(G1, Gc, Gr, f, p_*dt_,gradpar_, bmagInv_, ielectron);
  ...
  invert_implicit_terms(G1, Gc, Gr, f, r_*dt_,gradpar_, bmagInv_, ielectron);
  solver_->fieldSolve(G1, f);        
  if (pars_->dealias_kz) grad_par->dealias(f->phi);
  ...
  // G1 = inv(I - u_*dt*B)*G1
  invert_implicit_terms(G1, Gc, Gr, f, u_*dt_,gradpar_,bmagInv_, ielectron);
    solver_->fieldSolve(G1, f);          
  if (pars_->dealias_kz) grad_par->dealias(f->phi);
  // combine stage
  // compute A3 = A(G1)
