...
Solver_GK::Solver_GK(Parameters* pars, Grids* grids, Geometry* geo) :
  ...
  qneutFacBpar(nullptr), amperePerpFacPhi(nullptr), amperePerpFacBpar(nullptr), BparDenom(nullptr), qneutFacPhi_inv_l(nullptr), max_qneutFacPhi_inv(nullptr), max_qneutFacPhi_inv_l(nullptr), max_Jflr(nullptr), max_ampereParFac_inv(nullptr), max_qneutFacBpar_inv(nullptr), max_amperePerpFacPhi_inv(nullptr), max_amperePerpFacBpar_inv(nullptr)
{
  ...
  nn1 = grids_->Nyc;        nt1 = min(nn1, WARPSIZE );   nb1 = 1 + (nn1-1)/nt1;
  ...
  max_Jflr = (float**) malloc(sizeof(float*)*grids_->Nspecies);  
  ...
  if(pars_->fphi > 0.) {
    ...
    for(int is = 0; is < grids_->Nspecies; is++){
      ...
      cudaMalloc(&max_Jflr[is],    sizeof(float)*grids_->NxNyc*grids_->Nl);
      cudaMemset(max_Jflr[is], 0., sizeof(float)*grids_->NxNyc*grids_->Nl);    
