...
Lie_Trotter_Green::Lie_Trotter_Green(Linear *linear, Nonlinear *nonlinear, Solver *solver,
	     Parameters *pars, Grids *grids, Green *green, Geometry *geo, Forcing *forcing, double dt_in, const float gradpar, const float* kperp2) :
  linear_(linear), nonlinear_(nonlinear), solver_(solver), grids_(grids), pars_(pars),
  green_(green), geo_(geo), forcing_(forcing), dt_max(dt_in), dt_(dt_in), ielectron(-1), gradpar_(gradpar), kperp2_(kperp2)
{
  ...
  cublas_ = new Cublas_test(pars_, grids_, geo_, 0.0, 1.0, 0.0, true, (double) pars_->dt, A1[ielectron]->species->vt);
...
//  if (pars_->boundary_option_periodic) {
  ...
  if(true){
	...
	grad_par->zft_inverse(G1[is]);
      ...
      for(int is = 0; is<grids_->Nspecies; is++){
	apply_flr_phi<<<dG, dB>>>(phi_i, f->phi, kperp2, *(G[is]->species));
	grad_par->zft_nmoms(phi_i, phi_i, grids_->Nl);
	grad_par->zft(G[is]);
	compute_full_sol<<<dG, dB>>>(G[is]->G(), phi_i, grids_->kz, *(G[is]->species), sdt, gradpar_);
