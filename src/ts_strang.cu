...
Strang::Strang(Linear *linear, Nonlinear *nonlinear, Solver *solver,
...
void Strang::explicit_terms(MomentsG** A, MomentsG** G, Fields* f, bool setdt)
{
  for (int is=0; is<grids_->Nspecies; is++) {
    A[is]->set_zero();
    if(is == ielectron){
...
void Strang::implicit_terms(MomentsG** B, MomentsG** G, Fields* f)
{
  for (int is=0; is<grids_->Nspecies; is++) {
    B[is]->set_zero();
    if(is == ielectron){
...
void Strang::invert_bounce(MomentsG** G1, Fields *f, double sdt)
{
  if(pars_->fapar > 0.){
    for(int is = 0; is < grids_->Nspecies; is++){
...
void Strang::invert_streaming(MomentsG** G1, MomentsG** G0, Fields *f, double sdt,const float gradpar_, bool flip)
{
...
void Strang::ssprk3(MomentsG** A1, MomentsG** A2, MomentsG** A3, MomentsG** G, MomentsG** G1, Fields* f, bool setdt, float dt){
...
void Strang::invert_implicit_terms_linked_lw(MomentsG** G1, MomentsG** G0, Fields *f, double sdt,const float gradpar_, int ielectron)
{
  int max_iter_streaming = pars_->implicit_max_iter_streaming;
...
void Strang::advance(double *t, MomentsG** G, Fields* f)
{
  float a = 1. + sqrtf(2.)/2.;
...
//    invert_implicit_terms_linked_lw(G, G0, f, dt_,gradpar_, ielectron);
    ...
    invert_implicit_terms_linked_lw(G, G1, f, r_*dt_,gradpar_, ielectron);
    solver_->fieldSolve(G, f);
    ...
    invert_implicit_terms_linked_lw(G, G1, f, u_*dt_,gradpar_, ielectron);
    solver_->fieldSolve(G, f);
...
/*    invert_bounce(G, f, a*dt_/2.);
    solver_->fieldSolve(G, f);
    for(int is = ielectron; is < grids_->Nspecies; is++){
      A1[is]->set_zero();
      linear_->rhs_bounce(G[is], f, A1[is], dt_/2.);
      G[is]->add_scaled(1., G0[is], (1.-a)*dt_/2., A1[is]);
    }
    ...
    for(int is = ielectron; is < grids_->Nspecies; is++){
      A2[is]->set_zero();
      linear_->rhs_bounce(G[is], f, A2[is], dt_/2.);
      G[is]->add_scaled(1., G0[is], (1.-a)*dt_/2., A1[is], a*dt_/2., A2[is]);
    }*/
